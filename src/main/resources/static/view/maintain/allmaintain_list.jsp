...
<script type="text/javascript" src="<%=basePath%>view/maintain/listPage.js"></script>
<script type="text/javascript">
        function load() {
...
<body onload="load()">
	<div class="x-nav">
		<span class="layui-breadcrumb"> <a>流程管理</a> <a> <cite>报修批准</cite></a>
		</span> <a class="layui-btn layui-btn-small"
			style="line-height: 1.6em; margin-top: 3px; float: right"
			href="maintainController/showallmaintain.do" title="刷新"> <i
			class="layui-icon" style="line-height: 30px">ဂ</i></a>
	</div>
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so" method="post"
				action="maintainController/showallmaintain.do">
					<span>快速查询：</span>
		...
		<xblock> <span class="x-right" style="line-height: 40px">共有数据：${maintainsNum }
			条</span> </xblock>
				...
				<tr>
					<c:forEach items="${pageResult.dataList}" var="por"
						...
						class="layui-btn layui-btn-normal layui-btn-mini layui-btn-disabled"
									...
									<c:if test="${por.approveState == '同意'}">
						已启用
						</c:if> <c:if test="${por.approveState == '审核中'}">
						未批准
						</c:if>
							</span></td>
							...
							<td class="td-manage"><a class="layui-btn layui-btn-mini"
								onclick="member_stop(this,'${por.maintainId}'
								  )"
								href="javascript:;"
								<c:if test="${por.approveState == '同意'}">
						title="启用"
						 ...
						 title="未批准"
							...
							</a> <a title="删除" class="layui-btn  layui-btn-mini layui-btn-danger" onclick="member_del(this,'${por.maintainId}')"
	...
	/* 	function doNest() {
		...
		/*用户-控制*/
		function member_stop(obj,maintainId) {
			layer.confirm('确认要批准该申请吗？', function(index) {
				if ($(obj).attr('title') == '启用') {
					$.ajax({
						type:"post",
						url:"maintainController/updateStateDo.ajax",
						data:{
					...
					$.ajax({
						type:"post",
						url:"maintainController/updateState.ajax",
						data:{
							maintainId:maintainId
						},
						datatype:"text",
		...
		/*用户-删除*/
		function member_del(obj, maintainId) {
			layer.confirm('确认要删除吗？', function(index) {
				...
				window.location.href = "maintainController/deleteMaintain.do?maintainId="+maintainId+"";
